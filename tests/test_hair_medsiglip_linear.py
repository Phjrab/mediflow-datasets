...
def test_contract_is_one_validation_only_experiment(tmp_path):
    ...
    assert module.EXPERIMENTS == ["hair_medsiglip_448_frozen_linear_seed_42"]
    assert module.BASELINE["validation_accuracy"] == pytest.approx(0.7963258785942492)
    assert module.BASELINE["validation_macro_f1"] == pytest.approx(0.7957146810115047)
...
def test_selection_rule_uses_macro_f1_then_accuracy():
    record = {"validation": {"accuracy": 0.75, "macro_f1": 0.80}}
    assert module._selected(record)
    record = {
        "validation": {
            "accuracy": 0.80,
            "macro_f1": module.BASELINE["validation_macro_f1"],
        }
    }
    assert module._selected(record)
    record = {"validation": {"accuracy": 0.90, "macro_f1": 0.79}}
...
def test_notebook_embeds_current_sources_and_fixed_settings():
    ...
    for expected in (
        ...
        "hair_medsiglip_448_frozen_linear_seed_42",
        "hair_datasets.zip",
    ):
