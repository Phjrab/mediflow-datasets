...
실제 출처는 AI Hub [피부종양 이미지 합성 데이터](https://www.aihub.or.kr/aihubdata/data/view.do?currMenu=115&topMenu=100&dataSetSn=71864)
(데이터셋 71864)이다. 사이트는 15개 피부종양 클래스를 각 1,000장씩, 총 15,000장 제공한다.
MediFlow는 그중 10개 클래스를 선택했으므로 사이트 원천 기준으로는 10,000장이다.
초기 전처리 분할에는 각 클래스 900장씩 총 9,000장을 사용했다. 따라서 9,000장을 사이트 전체 수량으로 표현하지 않는다.
...
Skin 후보에는 범위 밖 입력 거부 기능이 없다. 출력 softmax 점수는 정답일 확률로
