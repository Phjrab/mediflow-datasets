...
- 서비스 의미: 이미 관찰 대상이 된 병변을 10개 학습 클래스 중 하나로 분류하는 후보 모델
...
- 서비스 의미: 관찰된 두피 상태를 다섯 범주로 분류

Softmax 최고 점수는 실제 정답 확률로 보정된 값이 아니다. 시스템에서는 `판정 보류`, `입력 오류`,
`모델 범위 밖`을 예측 결과와 구분한 별도 상태로 설계해야 한다.
...
Skin은 10개 병변 간 공개 데이터 성능은 높지만 실제 USB 현미경 사진으로 외부 검증하지 않았다.
공개 데이터의 배경, 조명과 촬영 배율이 실제 기기와 다르면 성능이 달라지는 **도메인 갭**이 남아 있다.
...
|---|---|---|
| Skin | `public_candidate_v1` B0 224 CE Augmented | 10개 클래스 순서 고정, RGB 0~255 |
| Web Skin | `public_candidate_v1_b0_256_ce` | 5개 클래스 순서 고정, 얼굴 웹캠용 |
| Hair | `public_candidate_v2_b1_384_ls005_adam` | 5개 클래스 순서 고정, 두피 현미경용 |
...
`status`는 적어도 `ok`, `no_result`, `invalid_input`, `out_of_scope`, `model_error`처럼 구분해야
한다. LLM이 빈 결과나 오류를 실제 예측으로 바꾸지 못하게 하기 위해서다.
