"""Build the standalone Web Skin MedSigLIP-448 linear-probe notebook."""
...
def build():
    source_names = ("common_engine", "common_workflow", "web_skin_medsiglip_linear")
    sources = {
...
from mediflow_datasets.web_skin_medsiglip_linear import run, summarize
"""
    cells = [
        cell(
            """# Web Skin 논문 기반 실험: MedSigLIP-448 Frozen Linear Probe

현재 배포 후보 `PMG·B0·256·CE v2`는 그대로 보존합니다. 이번 노트북은 Google의 의료
이미지·텍스트 사전학습 모델 **MedSigLIP-448**이 Web Skin 5개 클래스를 더 잘 분리하는지
Validation에서만 확인합니다. Test는 후보가 정해진 뒤 별도 노트북에서 한 번만 평가합니다.
...
**가설:** 일반 ImageNet 특징보다 의료 영상으로 사전학습된 MedSigLIP 특징이 얼굴 피부
질환의 미세한 차이를 더 잘 분리한다.

고정 조건은 동일 clean ZIP, 클래스 순서, Augmented Train 7,200장, Original Validation
500장, seed 42입니다. 변경 변수는 특징 추출기뿐입니다. MedSigLIP 본체는 학습하지 않고
고정한 뒤 분류용 Linear 층 하나만 50 epoch 학습합니다. 이 방식은 전체 미세조정보다 계산량과
과적합 위험이 작아 첫 선별 실험에 적합합니다.
"""
        ),
        cell(
            """DOMAIN = 'web_skin'
...
RUN_EXPERIMENTS = ['medsiglip_448_frozen_linear_seed_42']
""",
            True,
        ),
        cell(
...
처음에는 MedSigLIP 모델을 내려받고 7,700장의 특징을 추출하므로 시간이 걸립니다.
`train embedding: 128 / 7200` 같은 출력은 완료된 사진 수입니다. 캐시가 있으면 다음 실행에서
...
학습곡선, 혼동행렬, 기존 PMG v2와의 Accuracy·Macro F1 비교 그림과 보고서 ZIP을 만듭니다.
...
"""
    ...
    notebook = {
        ...
        "metadata": {
            ...
            "colab": {"name": "14_web_skin_medsiglip_linear_probe_colab.ipynb"},
            "accelerator": "GPU",
        },
        "nbformat": 4,
        "nbformat_minor": 5,
    }
    target = ROOT / "notebooks/14_web_skin_medsiglip_linear_probe_colab.ipynb"
