...
def prepare(config, profiles, sources, commit, local_parent="/content"):
    ...
    if c["mode"] in ("sam_screen", "final_candidate") and c["seeds"] != [42]:
    ...
    if c["mode"] in ("sam_screen", "final_candidate"):
        settings.update(
    ...
    if c["mode"] == "web_skin_paper_suite":
        settings.update(
            ...
            derm_model_id=c.get("derm_model_id"),
            derm_embedding_batch_size=c.get("derm_embedding_batch_size"),
        )
    ...
    if c["resume_dir"]:
        ...
        engine.write_json(
            output / "run_config.json",
            {
                ...
                "baseline": (
                    "Saved B0/256/CE Validation metrics reused; baseline not retrained"
                    if c["mode"] in ("web_skin_wsdan", "web_skin_paper_suite")
