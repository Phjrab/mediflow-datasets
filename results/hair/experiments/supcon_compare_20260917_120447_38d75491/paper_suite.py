...
def validate_suite(context):
    ...
    if config["domain"] != "hair" or config["mode"] != "paper_suite":
        raise ValueError("이 suite는 DOMAIN='hair', MODE='paper_suite' 전용입니다.")
    if config["train_variant"] != "augmented":
        raise ValueError("Hair 현재 후보와 같은 augmented Train을 사용해야 합니다.")
    if config["seeds"] != [42, 43, 44]:
        raise ValueError("SEEDS는 비교 계약에 따라 [42, 43, 44]여야 합니다.")
...
def summarize(context, records):
    ...
    summary = {
        ...
        "selection_metric": "validation_macro_f1_3seed_mean",
        "test_evaluated": False,
        "experiments": {},
    }
    for experiment_id in selected:
        ...
        if [item["spec"]["seed"] for item in items] != context["config"]["seeds"]:
            raise ValueError(experiment_id + "의 3-seed 결과가 완전하지 않습니다.")
    ...
    axes[0].set(
        ...
        title="Hair paper experiments: 3-seed mean ± population std",
    )
    ...
    print("Validation 전용 논문 실험 suite 완료:", output)
